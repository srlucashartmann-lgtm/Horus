  ...
  <div {...getRootProps()}>
    <InputWrapper ref={setAnchorEl} className={focused ? 'focused' : ''}>
      {value.map((option: FilmOptionType, index: number) => {
        const { key: _ignoredKey, ...tagProps } = getTagProps({ index });

        return <StyledTag label={option.label} {...tagProps} key={index} />;
      })}
      <input {...getInputProps()} />
    </InputWrapper>
  </div>
  {groupedOptions.length > 0 ? (
    <Listbox {...getListboxProps()}>
      {(groupedOptions as typeof data).map((option, index) => {
        const { key: _ignoredKey, ...optionProps } = getOptionProps({ option, index });
        return (
        ...
        <div {...getRootProps()}>
          <InputWrapper ref={setAnchorEl} className={focused ? 'focused' : ''}>
            {value.map((option: FilmOptionType, index: number) => {
              ...
              return <StyledTag label={option.label} {...tagProps} key={key} />;
            })}
            <input {...getInputProps()} />
          </InputWrapper>
        </div>
