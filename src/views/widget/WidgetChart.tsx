...
import IncomeChart, { Quantity, Slot } from 'sections/dashboard/analytics/IncomeChart';
...
export default function WidgetChart() {
  ...
  const [quantity, setQuantity] = useState<Quantity>('By volume');
  ...
  const handleChange = (event: MouseEvent<HTMLElement>, newAlignment: string) => {
    if (newAlignment) setSlot(newAlignment as Slot);
  };
