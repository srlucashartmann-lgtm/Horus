        ...
        <Grid container spacing={GRID_COMMON_SPACING} sx={{ p: { xs: 0, sm: 3.5 } }}>
          {pricingData.map((item, index) => (
            <Grid key={index} size={{ xs: 12, sm: 6, md: 4 }}>
              <MainCard content={false} sx={{ height: 1, p: 2.5, borderColor: withAlpha(colors[item.color], 0.5) }}>
                <MainCard content={false} border={false} sx={{ height: 1, bgcolor: withAlpha(colors[item.color], 0.1), p: 2.5 }}>
